...
### Prefilter
preFilter <- function(switchAnalyzeRlist, isoCount = 10, min.Count.prop = 0.7, 
    IFcutoff = 0.1, min.IF.prop = 0.5, acceptedGeneBiotype = NULL, 
    acceptedIsoformClassCode = NULL, removeSingleIsoformGenes = TRUE, 
    reduceToSwitchingGenes = FALSE, reduceFurtherToGenesWithConsequencePotential = FALSE, 
    onlySigIsoforms = FALSE, keepIsoformInAllConditions = FALSE, 
    alpha = 0.05, dIFcutoff = 0.1, quiet = FALSE,
    trace = FALSE) 
{
    # If tracing is requested, tictoc must be available
    if (trace) {
        if (!requireNamespace("tictoc", quietly = TRUE)) {
            stop("Package 'tictoc' is needed when trace = TRUE. Please install it.")
        }
        tictoc::tic.clearlog()
        tictoc::tic("Total filtering")
    }
    ...
    if (TRUE) {
        ...
        if (switchAnalyzeRlist$sourceId == "gtf") {
            warning(paste("The switchAnalyzeRlist seems to be created from a gtf file", 
                ...
                "please ignore this warning.", sep = " "))
        }
        if (switchAnalyzeRlist$sourceId == "preDefinedSwitches") {
        ...
        if (!is.null(acceptedIsoformClassCode)) {
            if (!"class_code" %in% colnames(switchAnalyzeRlist$isoformFeatures)) {
                stop(paste("The filter on class codes can only be used if", 
                  "the switchAnalyzeRlist was generated from cufflinks data"))
            }
        }
        if (!is.null(acceptedGeneBiotype) & "gene_biotype" %in% 
            ...
            if (!all(acceptedGeneBiotype %in% okBiotypes)) {
                ...
                warning(paste("Some of the supplied biotypes are not found in the isoforms supplied and will be ignored\n", 
        ...
        if (reduceToSwitchingGenes) {
            hasTest <- any(!is.na(c(switchAnalyzeRlist$isoformFeatures$isoform_switch_q_value, 
                switchAnalyzeRlist$isoformFeatures$gene_switch_q_value)))
            if (!hasTest) {
                stop(paste("The switchAnalyzeRlist does not contain the result", 
            ...
            if (alpha > 0.05) {
                warning(paste("Most journals and scientists consider an alpha larger", 
    ...
    if (TRUE) {
        # --- Data assembly ---
        if (trace) tictoc::tic("Data join")
        columnsToExtraxt <- c("iso_ref", "gene_ref", "isoform_id", 
            "gene_id", "gene_biotype", "condition_1", "condition_2", 
            "class_code", "gene_value_1", "gene_value_2", 
            "iso_overall_mean", "iso_value_1", "iso_value_2", 
            "IF_overall", "IF1", "IF2", "dIF", "gene_switch_q_value", 
            "isoform_switch_q_value")
        columnsToExtraxt <- na.omit(match(columnsToExtraxt, 
            colnames(switchAnalyzeRlist$isoformFeature)))
        isoformCountData <- switchAnalyzeRlist$isoformCountMatrix %>% 
            dplyr::rename_with(~paste0("Count_", .), -isoform_id)
        IFData <- switchAnalyzeRlist$isoformRepIF %>% 
            dplyr::rename_with(~paste0("IF_", .), -isoform_id)
        localData <- switchAnalyzeRlist$isoformFeature[, columnsToExtraxt]
        localData <- localData %>% 
            dplyr::inner_join(isoformCountData, by = "isoform_id") %>% 
            dplyr::inner_join(IFData, by = "isoform_id")
        transcriptCount <- length(unique(localData$isoform_id))
        if (trace) tictoc::toc()

        # --- Reduce to switching genes ---
        if (trace) tictoc::tic("Switch gene reduction")
        if (reduceToSwitchingGenes) {
            if (reduceFurtherToGenesWithConsequencePotential) {
                tmp <- extractSwitchPairs(switchAnalyzeRlist, 
                    alpha = alpha, dIFcutoff = dIFcutoff, onlySigIsoforms = onlySigIsoforms)
                deGenes <- unique(tmp$gene_ref)
            } else {
                isoResTest <- any(!is.na(localData$isoform_switch_q_value))
                if (isoResTest) {
                    deGenes <- localData$gene_ref[which(localData$isoform_switch_q_value < 
                        alpha & abs(localData$dIF) > dIFcutoff)]
                } else {
                    deGenes <- localData$gene_ref[which(localData$gene_switch_q_value < 
                        alpha & abs(localData$dIF) > dIFcutoff)]
                }
            }
            localData <- localData[which(localData$gene_ref %in% deGenes), ]
            if (!nrow(localData)) {
        ...
        # Define comparisons
        comparisons <- localData %>% 
            dplyr::select(condition_1, condition_2) %>% 
            dplyr::distinct()

        # --- Count filter (vectorised) ---
        if (trace) tictoc::tic("Count filter")
        if (!is.null(isoCount) && !is.null(min.Count.prop)) {
            localDataList <- lapply(seq_len(nrow(comparisons)), function(i) {
                comp <- comparisons[i, ]
                sampleIDs <- switchAnalyzeRlist$designMatrix %>%
                    dplyr::filter(condition %in% c(comp$condition_1, comp$condition_2)) %>%
                    dplyr::pull(sampleID)
                cols <- paste0("Count_", sampleIDs)
                cols <- intersect(cols, colnames(localData))
                sub <- localData[localData$condition_1 == comp$condition_1 &
                                 localData$condition_2 == comp$condition_2, ]
                if (nrow(sub) == 0) return(sub[0, ])
                mat <- as.matrix(sub[, cols] > isoCount)
                n_samples <- length(cols)
                keep <- rowSums(mat, na.rm = TRUE) >= max(3, min.Count.prop * n_samples)
                sub[keep, ]
            })
            localData <- dplyr::bind_rows(localDataList)
            if (!nrow(localData)) stop("No genes were left after filtering for gene expression")
        }
        if (trace) tictoc::toc()

        # --- IF filter (vectorised) ---
        if (trace) tictoc::tic("IF filter")
        if (!is.null(IFcutoff) && !is.null(min.IF.prop)) {
            localDataList <- lapply(seq_len(nrow(comparisons)), function(i) {
                comp <- comparisons[i, ]
                sampleIDs <- switchAnalyzeRlist$designMatrix %>%
                    dplyr::filter(condition %in% c(comp$condition_1, comp$condition_2)) %>%
                    dplyr::pull(sampleID)
                cols <- paste0("IF_", sampleIDs)
                cols <- intersect(cols, colnames(localData))
                sub <- localData[localData$condition_1 == comp$condition_1 &
                                 localData$condition_2 == comp$condition_2, ]
                if (nrow(sub) == 0) return(sub[0, ])
                mat <- as.matrix(sub[, cols] > IFcutoff)
                n_samples <- length(cols)
                keep <- rowSums(mat, na.rm = TRUE) >= max(3, min.IF.prop * n_samples)
                sub[keep, ]
            })
            localData <- dplyr::bind_rows(localDataList)
            if (!nrow(localData)) stop("No genes were left after filtering for gene expression")
        ...
        # --- Other filters ---
        if (trace) tictoc::tic("Other filters")
        if (!is.null(acceptedGeneBiotype)) {
            if ("gene_biotype" %in% colnames(localData)) {
                localData <- localData[which(localData$gene_biotype %in% 
                    acceptedGeneBiotype), ]
                if (!nrow(localData)) {
        ...
        if (!is.null(acceptedIsoformClassCode)) {
            localData <- localData[which(localData$class_code %in% 
        ...
        if (removeSingleIsoformGenes) {
            transcriptsPrGene <- split(localData$iso_ref, f = localData$gene_ref)
            transcriptsPrGene <- lapply(transcriptsPrGene, unique)
            genesToKeep <- names(transcriptsPrGene)[which(sapply(transcriptsPrGene, 
                function(x) length(x) > 1))]
            if (!length(genesToKeep)) {
                stop("No genes were left after filtering for mutlipe transcrip genes")
            }
            localData <- localData[which(localData$gene_ref %in% genesToKeep), ]
        }
        if (trace) tictoc::toc()
    }

    # ---- Final subsetting ----
    if (trace) tictoc::tic("Subsetting switchAnalyzeRlist")
    if (keepIsoformInAllConditions) {
        switchAnalyzeRlist <- subsetSwitchAnalyzeRlist(switchAnalyzeRlist, 
            switchAnalyzeRlist$isoformFeatures$isoform_id %in% localData$isoform_id)
    } else {
        switchAnalyzeRlist <- subsetSwitchAnalyzeRlist(switchAnalyzeRlist, 
    ...
    percentRemoved <- round(transcriptsRemoved/transcriptCount, 
        digits = 4) * 100
    if (!quiet) {
        message(paste("The filtering removed ", transcriptsRemoved, 
            " ( ", percentRemoved, "% of ) transcripts. There is now ", 
            transcriptsLeft, " isoforms left", sep = ""))
    }

    if (trace) tictoc::toc()   # Total filtering
    return(switchAnalyzeRlist)
}
