...
- [Rescude StringTie Annoation and Extract Gene Count Matrix]
...
- The dIFcutoff argument in the _switchPlot()_ function is what determines the "increased/decreased/unchage usage" underneed the isoform names in the transcript plots. Please refere to the isoform usage (bottom right) for indication of the statistical analysis.
...
### Rescude StringTie Annoation and Extract Gene Count Matrix
Due to the inherent difficulty of the transcript assembly problem StringTie output Gene_ids (MSTRG.XXXX) which corresponds to set of overlapping transcripts. This result in two separate problems:

- Some genes have been "merged" due to genomic overlap of transcripts from the different genes.
- Gene names (or non-StingTie gene_ids) have not been assigned.

From our experience with StringTie data there are typically tens of thousands of missing gene_names and ~50% of the missing gene_names are due to these problems (the rest are novel genes).

IsoformSwitchAnalyzeR now includes a "rescue" algorithm that can be used as follows:
...
```
From the *switchAnalyzeRlist* you can also run the switch analysis workflow as described in [Example Workflow] and examples of the results of this workflow can be found in [Examples Visualizations]. For more information about the details of the rescue please refere to the documentation of importRdata (by typing `?importRdata` in an R terminal), specifically the fixStringTie* arguments.

To make a differential expression analysis you can obtain the gene-count matrix from the *switchAnalyzeRlist* as follows:
...
This count matrix can be used as input to differential expression tools such as [edgeR](http://bioconductor.org/packages/edgeR/), [DESeq2](http://bioconductor.org/packages/DESeq2/), [limma](http://bioconductor.org/packages/limma/).
