...
Please note that if you had to submit your data as multiple runs at the SignalP website you can just supply a vector of strings indicating the path to each of the resulting files and IsoformSwitchAnalyzeR will read them all and integrate them.
...
analyzeIUPred2A() # OR
analyzeNetSurfP3()
# OR, to import Pfam + SignalP + MobiDB-lite results from a single InterProScan JSON file:
analyzeInterProScan()
...
Please note that CPC2 and CPAT performs the same type of analysis so only one of them is necessary. The same goes for IUPred2A and NetSurfP3. Similarly, analyzeInterProScan() can be used instead of analyzePFAM() and analyzeSignalP() (and, for the IDR portion, instead of analyzeIUPred2A()/analyzeNetSurfP3()) if you ran InterProScan instead of those tools separately.
...
Alternatively, if you ran InterProScan instead of Pfam/SignalP/(IUPred2A or NetSurfP-3) separately, a single call to *analyzeInterProScan()* imports whichever of those three analyses are present in the supplied JSON result file(s):

```{r}
### Add InterProScan analysis (alternative to analyzePFAM()+analyzeSignalP()+analyzeIUPred2A()/analyzeNetSurfP3() above)
exampleSwitchListAnalyzedInterProScan <- analyzeInterProScan(
    switchAnalyzeRlist            = exampleSwitchListIntermediary,
    pathToInterProScanResultFile  = system.file("extdata/interproscan_results.json.gz", package = "IsoformSwitchAnalyzeR"),
    showProgress = FALSE
)

exampleSwitchListAnalyzedInterProScan
```
