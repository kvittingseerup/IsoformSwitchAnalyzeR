...
We can look more into the details of the consequences by considering each type consequence separately as follows:
...
In summary: unless you have single end RNASeq data there is no good reason for not doing transcript level quantification. Summarizing transcript to gene level should be easy with tools such as [tximport](http://bioconductor.org/packages/release/bioc/html/tximport.html) or IsoformSwitchAnalyzeR's _importIsoformExpression()_ (which acutally is just a tximport wrapper with extra functionalities).
...
## Generating additional links for easy sharing:
