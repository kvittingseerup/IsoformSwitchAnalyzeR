...
-   **MobiDB-lite**: Please cite reference *21*.
-   **InterProScan**: Please cite reference *22*. If used to obtain Pfam, SignalP or MobiDB-lite results (via *analyzeInterProScan()*) please also cite the corresponding tool above.
...
1.  *Vitting-Seerup et al. **The Landscape of Isoform Switches in Human Cancers.** Mol. Cancer Res. (2017)* [link](http://mcr.aacrjournals.org/content/15/9/1206.long).
...
4.  *Vitting-Seerup et al. **spliceR: an R package for classification of alternative splicing and prediction of coding potential from RNA-seq data**. BMC Bioinformatics 2014, 15:81.* [link](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-15-81).
...
8.  *Finn et al. **The Pfam protein families database**. Nucleic Acids Research (2014) Database Issue 42:D222-D230* [link](https://academic.oup.com/nar/article/42/D1/D222/1062431).
9.  *Teufel et al. **SignalP 6.0 predicts all five types of signal peptides using protein language models**. Nat. Biotechnol (2022)* [link](https://www.nature.com/articles/s41587-021-01156-3).
...
20. *Vitting-Seerup. **Most protein domains exist as variants with distinct functions across cells, tissues and diseases**. NAR Genomics and Bioinformatics (2023)* [link](https://doi.org/10.1093/nargab/lqad084).
21. *Mehdiabadi et al. **MobiDB-lite 4.0: faster prediction of intrinsic protein disorder and structural compactness**. Bioinformatics (2025)* [link](https://doi.org/10.1093/bioinformatics/btaf297).
22. *Blum et al. **InterProScan 6: a modern large-scale protein function annotation pipeline**. Bioinformatics Advances (2026)* [link](https://doi.org/10.1093/bioadv/vbag141).
...
-   Pfam: Prediction of protein domains, which can be run locally using the pfam_scan.pl script as described in the readme found [here](https://ftp.ebi.ac.uk/pub/databases/Pfam/Tools/).
-   Tools for Coding Potential Analysis:
    -   CPC2: The Coding Calculator 2, which can be run either locally or via their [webserver](https://cpc2.gao-lab.org/batch.php).
...
-   SignalP: Prediction of Signal Peptides, which can be run either locally or via their [webserver](https://services.healthtech.dtu.dk/services/SignalP-6.0/) (v5 and v6 is supported).
...
-   InterProScan: Combined prediction of protein domains (Pfam), signal peptides (SignalP) and Intrinsically Disordered Regions (MobiDB-lite) in a single job, which can be run either locally or via their [webserver](https://www.ebi.ac.uk/interpro/search/sequence/). **This is the recommended way to obtain these three annotations**, replacing running Pfam, SignalP and IUPred2A/NetSurfP-3 separately - see [Importing External Sequence Analysis] below.
...
In a typical workflow, the user would here have to use the produced fasta files to perform the external analysis tools. We recommend running InterProScan, which in a single job predicts protein domains (Pfam), signal peptides (SignalP) and Intrinsically Disordered Regions (IDR, via MobiDB-lite). Alternatively these three types of annotation can be obtained by running Pfam, SignalP and IUPred2A/NetSurfP-3 separately - the main reason to do so is that IUPred2A additionally predicts Intrinsically Disordered Binding Regions (IDBR), which InterProScan/MobiDB-lite does not provide. In addition, CPAT or CPC2 (for prediction of coding potential, only one of them is necessary), DeepLoc2 (sub-cellular location) and DeepTMHMM (isoform topology) can be run. For more information on how to run those tools refer to [Advice for Running External Sequence Analysis Tools and Downloading Results]. To illustrate the workflow, we will here use the result of running some of these tools on the example data - these results are also included in the IsoformSwitchAnalyzeR package.
...
Alternatively to InterProScan, the same protein-domain, signal-peptide and IDR annotation can be obtained by running Pfam, SignalP and IUPred2A (or NetSurfP-3) separately:
...
Please note that if you had to submit your data as multiple runs at the InterProScan (or SignalP) website you can just supply a vector of strings indicating the path to each of the resulting files and IsoformSwitchAnalyzeR will read them all and integrate them.
...
```{r, fig.width=12.5, fig.height=7, warning=FALSE}
switchPlot(
    exampleSwitchListAnalyzed,
    gene='ZAK',
...
mySwitchList <- analyzeInterProScan( mySwitchList ) # recommended: Pfam + SignalP + IDR in one go
# OR, run these separately instead:
...
-   *createSwitchAnalyzeRlist()* is the central (internal) constructor of the switchAnalyzeRlist class, used internally by all the *import\<description\>()* functions to build a switchAnalyzeRlist from their parsed input. It is not exported and should not be called directly -- use one of the *import\<description\>()* functions instead (most commonly *importRdata()*). Note that this function also adds the 'iso_ref' and 'gene_ref' columns to the isoformFeatures entry of a switchAnalyzeRlist. These reference indexes give a unique id to each gene and isoform in each comparison and thereby allow for easy integration of the different downstream analyses.
...
An alternative to using *importRdata()* is *importGTF()*.
...
-   If you are planning on using the webserver version of the external sequence analysis tools you should set *removeLongAAseq=TRUE* and *alsoSplitFastaFile=TRUE*. These arguments exist because some webservers (SignalP and InterProScan) have (quite) strict limits on the number of amino acid sequences one can submit at a time - InterProScan in particular limits submissions to 100 sequences, which is why *maxFastaFileSize* defaults to 100. When those arguments are true IsoformSwitchAnalyzeR will generate multiple filtered amino acid fasta files which can then be supplied as multiple individual analysis at the websites.
...
-   CPC2 : The Coding Potential Calculator 2 which is a tool for predicting whether an isoform is coding or not. CPC2 can be run either locally or via their [webserver](https://cpc2.gao-lab.org/batch.php). This is an alternative to CPAT.
-   Pfam : Prediction of protein domains, which can be run locally using the pfam_scan.pl script as described in the readme found [here](https://ftp.ebi.ac.uk/pub/databases/Pfam/Tools/).
-   SignalP : Prediction of Signal Peptides -- a short N-terminal sequence of a peptide indicating where a protein should be membrane bound or secreted. SignalP can be run either locally or via their [webserver](https://services.healthtech.dtu.dk/services/SignalP-6.0/) (v5 and v6 is supported).
...
-   DeepTMHMM : Prediction of protein topology -- the proteins location compared to the cell membrane. Can be intracellular, transmembrane (TM) or extracellular.
-   DeepLoc2 : Prediction of sub-cellular localization(s) of protein.
-   InterProScan : Combined prediction of protein domains (Pfam), signal peptides (SignalP) and Intrinsically Disordered Regions (MobiDB-lite) in a single job. Can be run either locally or via their [webserver](https://www.ebi.ac.uk/interpro/search/sequence/). **Recommended** over running Pfam, SignalP, and (partially) IUPred2A/NetSurfP-3 separately.
...
-   Some webservers (SignalP, NetSurfP-3, IUPred2A and InterProScan) have (quite) strict limits on the number (and length) of amino acid sequences one can submit at the time - InterProScan in particular limits submissions to 100 sequences. As indicated above extractSequence() can generate multiple filtered amino acid fasta files (batched to at most *maxFastaFileSize* sequences each, which defaults to 100 to match InterProScan's limit) that can be uploaded one at the time (aka as multiple runs) to circumvent this.
...
-   CPC2 : Use default parameters and if required select the most similar species. If the [webserver](https://cpc2.gao-lab.org/batch.php) (batch submission) was used, download the tab-delimited result file (via the "Download the result" button). If a stand-alone version was just just supply the path to the result file.
-   Pfam : Use default parameters and the amino acid fasta file (\_AA.fasta). Run locally using the pfam_scan.pl script as described in the readme found [here](https://ftp.ebi.ac.uk/pub/databases/Pfam/Tools/) and supply the path to the result file. A more detailed walkthrough is found under details in the documentation of the analyzePFAM() function (?analyzePFAM). Note: the classic Pfam webserver no longer supports batch submissions - if you don't want to run Pfam locally, use InterProScan instead (see below).
-   SignalP : Use the amino acid fasta file (\_AA.fasta). If using the [webserver](https://services.healthtech.dtu.dk/services/SignalP-6.0/) SignalP should be run with the parameter "Short output (no figures)" under "Output format" and one should select the appropriate "Organism group". When using a stand-alone version SignalP should be run with the '-f summary' option. If using the webserver the results can be downloaded using the "Downloads" button in the top-right corner where the user should select "Prediction summary" and supply the path to the resulting file to the "pathToSignalPresultFile" argument. If a stand-alone version was just supply the path to the summary result file.
...
-   DeepTMHMM : Can be run from from this [website](https://biolib.com/DTU/DeepTMHMM) and afterwards all files can be downloaded as a "gff3 format" file can be used as input to this function.
-   DeepLoc2 : Can be run from this [website](https://services.healthtech.dtu.dk/service.php?DeepLoc-2.0) and choose "Short output (no figures)". Once it is done you can download by pressing the "Download prediction results: CSV Summary" at the top of the result section (just above the red "Probability thresholds" button).
-   InterProScan : 1) Go to the [webserver](https://www.ebi.ac.uk/interpro/search/sequence/) (or run the stand-alone tool locally). 2) Upload the amino acid fasta file (\_AA.fasta) created with extractSequence() - remember only up to 100 sequences can be submitted at a time (the default *maxFastaFileSize* in extractSequence()). 3) Submit your job and wait for it to complete. 4) Download the result in JSON format and supply the path to that file to the "pathToInterProScanResultFile" argument - if you had to split your sequences into multiple batches just supply all the resulting JSON files as a vector of strings. A more detailed walkthrough is found under details in the documentation of the analyzeInterProScan() function (?analyzeInterProScan).
...
# Recommended: import Pfam + SignalP + MobiDB-lite results from a single InterProScan JSON file
analyzeInterProScan()
# OR, run these separately instead:
analyzePFAM()
analyzeSignalP()
...
Please note that CPC2 and CPAT performs the same type of analysis so only one of them is necessary. The same goes for IUPred2A and NetSurfP3. We recommend using analyzeInterProScan() instead of analyzePFAM() and analyzeSignalP() (and, for the IDR portion, instead of analyzeIUPred2A()/analyzeNetSurfP3()), since it obtains all three annotations from a single InterProScan run.
...
### Add InterProScan analysis (recommended: imports Pfam + SignalP + MobiDB-lite results in one call)
...
```{r, eval = FALSE}
### Add PFAM analysis
exampleSwitchListAnalyzedSeparateTools <- analyzePFAM(
    switchAnalyzeRlist   = exampleSwitchListIntermediary,
    pathToPFAMresultFile = system.file("extdata/pfam_results.txt", package = "IsoformSwitchAnalyzeR"),
    showProgress=FALSE
)
...
exampleSwitchListAnalyzedSeparateTools <- analyzeSignalP(
    switchAnalyzeRlist       = exampleSwitchListAnalyzedSeparateTools,
    pathToSignalPresultFile  = system.file("extdata/signalP_results.txt", package = "IsoformSwitchAnalyzeR")
)

### Add IUPred2A analysis
exampleSwitchListAnalyzedSeparateTools <- analyzeIUPred2A(
    switchAnalyzeRlist        = exampleSwitchListAnalyzedSeparateTools,
    pathToIUPred2AresultFile = system.file("extdata/iupred2a_result.txt.gz", package = "IsoformSwitchAnalyzeR"),
    showProgress = FALSE
)
...
exampleSwitchListAnalyzedSeparateTools <- analyzeNetSurfP3(
    switchAnalyzeRlist = exampleSwitchListAnalyzedSeparateTools,
...
Please note that if you had to submit your data as multiple runs at the SignalP or InterProScan website you can just supply a vector of strings indicating the path to each of the resulting files to the functions above and IsoformSwitchAnalyzeR will read them all and integrate them.
...
1)  After QC and potential quality trimming you you map reads to the genome (with tools such as [STAR](https://github.com/alexdobin/STAR/blob/master/doc/STARmanual.pdf), [HISAT2](https://daehwankimlab.github.io/hisat2/manual/) etc.).
