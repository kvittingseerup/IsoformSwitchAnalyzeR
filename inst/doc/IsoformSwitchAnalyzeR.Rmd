...
Generally we would not recommend analysis of experiments without replicates as they will always be descriptive of that particular experiment (aka have no power to tell if it would be the same next time the experiment was run). But since pilot studies can be very elucidating IsoformSwitchAnalyzeR can be used. The main thing you to do is to solely rely on the dIF values to call isoform switches. This can be done by following the steps outlined in [Detailed Workflow] with a few modifications:
